...
│ │ ○ Monthly (1st of month)            │ │ Next: Jan 9, 2026   │ │
...
1. **Implement dual table swap for stock prices and technical indicators** *(Critical, 11/28/25)*
...
2. **Add cryptocurrency ticker support** *(Data Expansion, 12/5/25)*
...
3. **Complete type hint coverage** *(Code Quality, 12/5/25)*
...
4. **Build service communication framework** *(Foundation, 12/12/25)*
...
5. **Implement initial MarketDataService** *(Core Service, 12/19/25)*
...
6. **Build Qt 6 UI framework foundation** *(Application Structure, 1/9/26)*
...
7. **Build stock price visualization UI** *(Core Feature, 1/16/26)*
...
8. **Implement SEC filings pipeline** *(Data Foundation, 1/23/26)*
...
9. **Implement initial CompanyDataService** *(Service Layer, 1/30/26)*
...
10. **Add company summaries to UI** *(AI Processing, 2/6/26)*
...
11. **Generate company KPIs** *(Data Processing, 2/13/26)*
...
12. **Extend CompanyDataService with KPI support** *(Service Extension, 2/20/26)*
...
13. **Add company KPIs to UI** *(UI Enhancement, 2/27/26)*
...
14. **Implement news scraping pipeline** *(Data Collection, 3/6/26)*
...
15. **Generate news sentiment analysis** *(AI Processing, 3/13/26)*
...
16. **Add news sentiment to UI** *(UI Integration, 3/20/26)*
...
17. **Create portfolio tracking database schema** *(Data Foundation, 4/3/26)*
...
18. **Implement backtesting framework** *(Analysis Foundation, 4/17/26)*
...
19. **Implement rebalancing scheduler** *(Automation, 4/24/26)*
...
20. **Implement standard allocation templates** *(Portfolio Management, 5/1/26)*
...
21. **Implement asset class categorization** *(Classification Framework, 5/8/26)*
...
22. **Implement quadratic programming library** *(Optimization Engine, 5/22/26)*
...
23. **Add portfolio management to UI** *(Complete Integration, 6/5/26)*
...
### **Final Integration**
24. **Validate setup instructions** *(Documentation, 6/19/26)*
    ...
    - Create Python installation script for Linux/Ubuntu systems that: analyzes system resources (memory/CPU cores) and generates appropriate default values for settings files, creates required log directories, handles dynamic database tablespace paths, provides system resource sanity checks, and addresses database security concerns

### **Roadmap Notes**
- **Total Tasks**: 24 tasks organized into 8 functional modules
- **Estimated Timeline**: mid June 2026 + 3 months buffer for contingency. With an overall target date of 15 September 2026.
...
### **Possible Future Enhancements (Post-September 2026)**
- **Conference Call & Investor Meeting Analysis**: Potential expansion of sentiment analysis to include earnings call transcripts, annual investor meetings, and conference presentations (contingent on finding free data sources and audio-to-text solutions)
- **Enhanced AI Integration**: Model Context Protocol (MCP) evaluation and integration for standardized financial data access patterns
- **Cross-Platform Deployment**: Windows and macOS validation and optimization beyond initial Linux/Ubuntu focus
